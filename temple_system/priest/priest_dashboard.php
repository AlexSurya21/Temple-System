...
if ($conn) {
    ...
    if ($stmt = @$conn->prepare("SELECT COUNT(*) AS c FROM booking WHERE status = 'pending'")) {
        $stmt->execute();
        $res = $stmt->get_result()->fetch_assoc();
        ...
        <?php else: ?>
            <?php foreach ($recentBookings as $b): ?>
                <div class="recent-item">
                    <strong><?php echo htmlspecialchars($b['full_name']); ?></strong> booked <?php echo htmlspecialchars($b['pooja_name']); ?>
                    <br>Date: <?php echo htmlspecialchars($b['booking_date']); ?> | Time: <?php echo htmlspecialchars($b['booking_time']); ?>
                    <br>Status: <?php echo ucfirst($b['status']); ?>
                </div>
            <?php endforeach; ?>
        <?php endif; ?>
    </div>
