...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($email) || empty($password)) {
        ...
        $stmt = $conn->prepare("SELECT admin_id, admin_name, email, password 
                                FROM admin 
                                WHERE email = ? 
                                LIMIT 1");
        if (!$stmt) {
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .temple-icon { font-size: 60px; margin-bottom: 15px; }
        .login-header h1 { color: #764ba2; font-size: 28px; margin-bottom: 10px; }
        ...
        .password-toggle .toggle-btn:hover { color: #764ba2; }
        ...
        .form-footer { margin-top: 20px; text-align: center; color: #666; font-size: 13px; }

        @media (max-width: 480px) {
            .login-container { padding: 30px 20px; }
            .login-header h1 { font-size: 24px; }
            .temple-icon { font-size: 50px; }
        }
    </style>
</head>
<body>
    <div class="login-container">
        ...
        <form method="POST" action="">
            ...
            <button type="button" class="home-btn" onclick="window.location.href='../index.php'">⬅ Back to Home</button>
        </form>
